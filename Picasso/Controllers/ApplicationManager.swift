...
struct SBApp {
        ...
        for iconName in pngIconPaths {
            autoreleasepool {
                ...
                if let data = try? Data(contentsOf: urlOfOriginal) {
                    // Has backup
                    try? KFD.overwriteFile(at: iconURL.path, with: data)
    ...
    func restoreCatalog() throws {
        try autoreleasepool {
            ...
            try KFD.overwriteFile(at: catalogURL.path, with: try Data(contentsOf: catalogBackupURL), unlockDataAtEnd: !isSystem)
            ...
            if bundleURL.lastPathComponent != "MobileTimer.app" {
                do {
                    ...
                    do {
                        ...
                        throw KFD.KFDOverwriteError.corruption
                    }
                } catch {
    ...
    func setPNGIcons(icon: ThemedIcon) throws {
        for iconName in pngIconPaths {
            try autoreleasepool {
                ...
                try? KFD.overwriteFile(at: iconURL.path, with: cachedIcon!, unlockDataAtEnd: !isSystem)
