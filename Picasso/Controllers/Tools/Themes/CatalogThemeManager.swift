        ...
        for (i,change) in changes.enumerated() {
            do {
                ...
                if didApply == true && !verifyCatalog(change) {
                    errors.append(KFD.KFDOverwriteError.corruption.localizedDescription)
                    KFD.isKFDSafe = false
                    break
                }
            } catch {
                errors.append(error.localizedDescription)
                if error is KFD.KFDOverwriteError {
                    break
                }
            }
                ...
                if UserDefaults.standard.bool(forKey: "catalogIconTheming") {
                    if FileManager.default.fileExists(atPath: catalogURL.path) {
                        ...
                        if let catalogIconName = app.catalogIconName() { // happens with app which store their icons as .png's inside bundle
                            ...
                            try KFD.overwriteFile(at: catalogURL.path, with: try Data(contentsOf: newCatalogURL), multipleIterations: true)
                            return true
                        }
                    }
                }
...
//        let success = KFD.overwriteFile(at: url.path, with: Data([UInt8](repeating: 1, count: byteArray.count )))
//        print(success)
...
//
//        let success = KFD.overwriteFile(at: url.path, with: newData)
//        if !success {
