...
struct PicassoApp: App {
    ...
    @StateObject var picassoAPI = PicassoAPI()
    ...
    @State var showingSelectExploitMethodAlert = false
    
    var body: some Scene {
        WindowGroup {
            RootView()
                .environmentObject(picassoAPI)
                .onAppear {
                    if UserDefaults.standard.string(forKey: "userToken") == nil {
                        showLogin = true
                    } else {
                        showExploitSelectionAlert()
                    }
...
#endif
                    // credit: TrollTools
//                    if !catalogFixupShown, let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String, let url = URL(string: "https://api.github.com/repos/leminlimez/Picasso/releases/latest") {
...
//                                    UIApplication.shared.confirmAlert(title: "Update available", body: "Picasso \(json["tag_name"] as? String ?? "update") is available, do you want to visit releases page?", onOK: {
//                                        UIApplication.shared.open(URL(string: "https://github.com/leminlimez/Picasso/releases/latest")!)
//                                    }, noCancel: false)
//                                }
//                            }
//                        }
...
//                    }
                ...
                .onOpenURL(perform: { url in
                    ...
                    if url.absoluteString == "picasso://dateset" {
                        createStatusBarDateTag()
                        return
                    }
                    // MARK: Status Bar
                    if url.absoluteString.starts(with: "picasso://statusbar") {
                        // reset status bar
                        if url.absoluteString == "picasso://statusbar:reset" {
            ...
            //                }
                ...
                .sheet(isPresented: $showLogin, content: { LoginView(onLogin: showExploitSelectionAlert) })
                .alert("Select exploit", isPresented: $showingSelectExploitMethodAlert, actions: {
                    Button("smith", role: .cancel) {
                        KFD.puaf_method = 1
                        exploit()
                    }
                    Button("physpuppet") {
                    ...
                    Text("smith: iOS 16.0 - 16.5, 16.6b1\nphyspuppet: iOS 16.0 - 16.3.1")
                })
        }
    }
    
    func showExploitSelectionAlert() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            showingSelectExploitMethodAlert = true
        }
    }
    
    func exploit() {
        UIApplication.shared.alert(title: "Exploiting...", body: "Praying to RNGesus"/*messages[(String(Int.random(in: 0...100000)).count - 1) / 2]*/, withButton: true)
