...
struct CatalogFixupView: View {
    ...
    var body: some View {
        VStack {
            ...
            Text(showSuccess ? "Your apps should now function properly" : "Please wait for Picasso to finish theming...")
            ...
            Spacer()
            if !showSuccess { Text("In the meantime you can ...", comment: "A joke about users being able to join Discord while they wait for Picasso to finish theming. \"In the meantime you can Join Discord\" is the text for button") }
            Button("Join Discord") {
            ...
            if showSuccess {
                Button("Close") {
                    if !restartRequired {
                        ...
                        UIApplication.shared.confirmAlert(title: "App restart required", body: "You've applied a theme which contains an icon for Picasso itself. Please restart for app to work.", onOK: {
                            exitGracefully()
                        }, noCancel: true)
                    }
                }
