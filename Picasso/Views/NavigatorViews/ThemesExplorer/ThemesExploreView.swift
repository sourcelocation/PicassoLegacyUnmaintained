...
struct ThemesExploreView: View {
    ...
    @ObservedObject var picassoAPI = PicassoAPI.shared
                            ...
                            ForEach(themes) { theme in
                                if searchTerm == "" || theme.name.lowercased().contains(searchTerm.lowercased()) || (theme.contact.values.first ?? "Unknown author").lowercased().contains(searchTerm.lowercased()) {
                                    Button {
                                        ...
                                        VStack(spacing: 0) {
                                            CachedAsyncImage(url: picassoAPI.getPreviewURLForTheme(theme: theme), urlCache: .imageCache) { image in
                                                image
                                                    .resizable()
    ...
    func loadThemes() {
        Task {
            do {
                themes = try await picassoAPI.fetchThemes(type: themeTypeShown)
                themes = picassoAPI.filterTheme(themes: themes, filterType: filterType)
            } catch {
    ...
    func downloadTheme(theme: DownloadableTheme) {
        ...
        Task {
            do {
                try await picassoAPI.downloadTheme(theme: theme)
