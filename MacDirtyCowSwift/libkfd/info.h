...
void getiPhoneModel(char* model, size_t maxLength) {
    int name[] = {CTL_HW, HW_MACHINE};
    size_t length = maxLength;
    sysctl(name, 2, model, &length, NULL, 0);
}
...
void info_init(struct kfd* kfd)
{
    ...
    for (u64 i = 0; i < number_of_kern_versions; i++) {
        ...
        char device_id[256];
        getiPhoneModel(device_id, sizeof(device_id));
        
        if (!memcmp(kfd->info.env.kern_version, current_kern_version, strlen(current_kern_version)) && !memcmp(current_device_id, device_id, min(strlen(current_device_id), strlen(device_id)))) {
            // check for device model if it matches the current device's
